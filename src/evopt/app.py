...
battery_config_model = api.model(
    "BatteryConfig",
    {
        "charge_from_grid": fields.Boolean(
            required=False,
            description="Controls whether the battery can be charged from the grid.",
        ),
        "discharge_to_grid": fields.Boolean(
            required=False,
            description="Controls whether the battery can discharge to grid.",
        ),
        ...
        "s_goal": fields.List(
            fields.Float,
            required=False,
            description="Goal state of charge at each time step (Wh)",
        ),
        ...
        "p_a": fields.Float(
            required=True,
            description="Monetary value per Wh at end of the optimization horizon",
        ),
    },
)

time_series_model = api.model(
    "TimeSeries",
    {
        "dt": fields.List(
            fields.Float,
        ...
        "gt": fields.List(
            fields.Float,
            required=True,
            description="Required energy for home consumption at each time step (Wh)",
        ),
        "ft": fields.List(
            fields.Float,
            required=True,
            description="Forecasted solar generation at each time step (Wh)",
        ),
        "p_N": fields.List(
            fields.Float,
            required=True,
            description="Price per Wh taken from grid at each time step",
        ),
        "p_E": fields.List(
            fields.Float,
            required=True,
            description="Remuneration per Wh fed into grid at each time step",
        ),
    },
)

optimization_input_model = api.model(
    "OptimizationInput",
    {
        "strategy": fields.Nested(strategy_model, required=False, description="Optimization strategy"),
        "batteries": fields.List(
            fields.Nested(battery_config_model),
            required=True,
...
optimization_result_model = api.model(
    "OptimizationResult",
    {
        ...
        "batteries": fields.List(
            fields.Nested(battery_result_model),
            description="Battery optimization results",
        ),
...
class OptimizeCharging(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            strategy = OptimizationStrategy(charging_strategy=charging_strat, discharging_strategy=discharging_strat)
